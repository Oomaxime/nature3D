...
function setInstancerCount(instancer: Instancer, count: number) {
  for (const im of instancer.parts)
    im.count = Math.min(count, instancer.maxCount);
}
...
function placementsToInstancer(
  instancer: Instancer,
...
export default class Props {
  ...
  constructor(
    ...
    const stumpP = samplePlacements(
      sampler,
      STUMP_COUNT,
      LAKE_INNER_RADIUS + 6,
      () => {
        const s = 0.8 + Math.random() * 0.5;
        return new THREE.Vector3(s, s * (0.85 + Math.random() * 0.3), s);
      },
    );
    const logP = samplePlacements(
      sampler,
      LOG_COUNT,
      LAKE_INNER_RADIUS + 6,
      () => {
    ...
    const boulderP = samplePlacements(
      sampler,
      BOULDER_COUNT,
      LAKE_INNER_RADIUS + 4,
      () => {
        const s = 0.4 + Math.random() * 1.2;
        return new THREE.Vector3(s, s * (0.7 + Math.random() * 0.5), s);
      },
    );

    loader.load(
      "/models/TreeStump024/TreeStump024_Blender_Cycles.glb",
      (gltf) => {
        this.stumpInstancer = buildInstancer(gltf.scene, STUMP_COUNT, scene);
        placementsToInstancer(
          this.stumpInstancer,
          stumpP,
          getYFloor(gltf.scene),
        );
    ...
    loader.load(
      "/models/RockBoulderLarge054/RockBoulderLarge054_Blender_Cycles.glb",
      (gltf) => {
        this.boulderInstancer = buildInstancer(
          gltf.scene,
          BOULDER_COUNT,
          scene,
        );
        placementsToInstancer(
          this.boulderInstancer,
          boulderP,
          getYFloor(gltf.scene),
        );
  ...
  setupGui(gui: GUI) {
    ...
    folder
      .add(params, "stumps", 0, STUMP_COUNT, 1)
      .name("Stumps")
      .onChange(
        (v: number) =>
          this.stumpInstancer && setInstancerCount(this.stumpInstancer, v),
      );
    folder
      .add(params, "logs", 0, LOG_COUNT, 1)
      .name("Logs")
      .onChange(
        (v: number) =>
          this.logInstancer && setInstancerCount(this.logInstancer, v),
      );
    folder
      .add(params, "boulders", 0, BOULDER_COUNT, 1)
      .name("Boulders")
      .onChange(
        (v: number) =>
          this.boulderInstancer && setInstancerCount(this.boulderInstancer, v),
      );
  }
