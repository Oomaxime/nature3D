...
interface Instance { x: number; z: number; rotY: number; scale: number }
...
// ── Initial placements ────────────────────────────────────────────────────────
// Z1 = top-left shore (back-left, where the water edge is irregular)
// Z2 = bottom-front shore (facing the camera, where water fades out)
//
// All Y positions are computed automatically from getTerrainHeight().
// Adjust via GUI — report back the values you like.
...
const GROUPS: PropGroup[] = [
  {
    ...
    instances: [
      { x: -18.6, z: -17.2, rotY: 4.60, scale: 2.5 },  // Z1·R0
      { x: -17.2, z: -18.6, rotY: 0.00, scale: 2.5 },  // Z1·R1
      { x:  -1.8, z: -25.6, rotY: 1.90, scale: 1.0 },  // Z1·R2
      { x: -12.6, z:  21.0, rotY: 4.85, scale: 1.5 },  // Z2·R3
      { x: -24.0, z:   8.0, rotY: 4.50, scale: 3.0 },  // Z2·R4
    ],
  },
]
...
export default class ShoreProps {
  ...
  constructor(scene: THREE.Scene, renderer: THREE.WebGLRenderer, camera: THREE.Camera) {
    ...
    for (const group of this.groups) {
      loader.load(group.path, (gltf) => {
        ...
        gltf.scene.traverse(c => { if (!src && c instanceof THREE.Mesh) src = c })
        if (!src) return
  ...
  setupGui(gui: GUI) {
    ...
    const zoneLabel = (groupName: string, i: number) => {
      const thresholds: Record<string, number> = { Rocks: 3, Logs: 1, Stumps: 1 }
    ...
    for (const group of this.groups) {
      ...
      for (let i = 0; i < group.instances.length; i++) {
        ...
        f.add(inst, 'rotY',    0, Math.PI*2, 0.05).name('RotY').onChange(update)
