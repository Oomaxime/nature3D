...
export const LAKE_SURFACE_Y = -0.5;
...
function computeHeight(x: number, y: number): number {
  ...
  h += Math.sin(x * 0.3 + 3.1) * Math.cos(y * 0.28 + 1.5) * 1.2;
  ...
  if (dist < LAKE_OUTER_RADIUS) {
    const t = Math.max(
      0,
      (dist - LAKE_INNER_RADIUS) / (LAKE_OUTER_RADIUS - LAKE_INNER_RADIUS),
    );
...
export default class Terrain {
  ...
  constructor(scene: THREE.Scene) {
    ...
    const BASE =
      "/textures/floor/Poliigon_GrassPatchyGround_4585(1)/1K/Poliigon_GrassPatchyGround_4585_";
